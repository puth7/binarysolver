...
for m in list_m:
  ...
  xlabel='Number of blanks '
  r.png(file = '"%s"'%name )
  r.plot(blnk_lst,com_time_lst, log='"y"',main='"%s"'%mtitle, ylab='"%s"'%ylabel, xlab='"%s"'%xlabel)
  r.dev_off()
...
for m in list_m:
  ...
  blnk_lst = [1..(2*m)^2 ] 
  ratio_uniq_lst   = [float(uniq[m, blnk].count(False))/len(uniq[m, blnk])  for blnk in [1..(2*m)^2] ]
  name = 'simulation_plots/uniq_scatter_plot_'+str(m) + '.png'
  mtitle='Distribution of uniqueness\n Size: '+str(2*m)+' x '+str(2*m)
  ylabel='Ratio of uniqueness'
  xlabel='Blanks '
  r.png(file = '"%s"'%name )
  r.plot(blnk_lst,ratio_uniq_lst, main='"%s"'%mtitle, xlab='"%s"'%xlabel, ylab='"%s"'%ylabel)
  r.dev_off()
...
for m in list_m:
  ...
  #   except:
  #     continue
  blnk_lst = [blnk for blnk in [2..max(zip(*sorted(exh))[1])] for i in xrange(len(exh[m,blnk,'time']))]  
  time_lst = flatten([exh[m, blnk, 'time'] for blnk in [2..max(zip(*sorted(exh))[1])]])
  guess_lst = flatten([exh[m, blnk, 'guess'] for blnk in [2..max(zip(*sorted(exh))[1])]])
  bcktrk_lst = flatten([exh[m, blnk, 'bcktrk'] for blnk in [2..max(zip(*sorted(exh))[1]) ]])
